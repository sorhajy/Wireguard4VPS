...
read -r -p "Enter the endpoint (external ip and port) in format [ipv4:port] (e.g. 4.3.2.1:51820):" ENDPOINT
if [ -z "$ENDPOINT" ]
then
  ENDPOINT="$ext_ip"
...
cat "./endpoint.var" | sed -e "s/:/ /" | while read -r SERVER_EXTERNAL_IP SERVER_EXTERNAL_PORT
do
